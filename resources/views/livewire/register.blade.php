<form method="POST" action="{{ route('register') }}" novalidate>
    ...
    <div class="{{ $currentStep != 1 ? 'hidden' : '' }}">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-2">
                            {!! __('Я согласен с :terms_of_service и :privacy_policy', [
                                    'terms_of_service' => '<a target="_blank" href="'.route('terms.show').'" class="underline text-sm text-gray-600 hover:text-gray-900">Условия использования</a>',
                                    'privacy_policy' => '<a target="_blank" href="'.route('policy.show').'" class="underline text-sm text-gray-600 hover:text-gray-900">Политика конфиденциальности</a>',
                            ]) !!}
                        </div>

                        @error('terms')<span class="text-red-400">{{ $message }}</span>@enderror
                    </div>
                </x-jet-label>
            </div>
        @endif
        ...
        <div class="flex items-center justify-end mt-4">
            <x-jet-button class="ml-4" wire:click.prevent="submitForm">
                {{ __('Register') }}
            </x-jet-button>
        </div>
